...
<body>
 ...
 <?php include("navigation.php");?>
  ...
  <!-- Header -->
  <header class="bg-primary pt-4 pb-1 mb-1">
    <div class="container ">
      <div class="row">
        <div class="col-lg-12">
  ...
  <!-- Page Content -->
 	...
 	<div class='container'>		<div><p class="lead text-primary">Sort By:-</p><br></div>
		  <div class="row">
		    <form method="POST" action="workerbookinghistory.php" id="bookingstoday">
			 <div class="col-md-3">
			 <?php
			 ...
			 <form method="POST" action="workerbookinghistory.php" id="bookingsdone">
			 <div class="col-md-3">
				<?php
			 ...
			 <form method="POST" action="workerbookinghistory.php" id="bookingspending">			
			 <div class="col-md-3">
			 <?php
			 ...
			 <form method="POST" action="workerbookinghistory.php" id="bookingsbypayment">
			 <div class="col-md-3">
						...
						(SELECT customerLogin.phone FROM customerLogin WHERE customerLogin.id=booking.cid) AS customer_phone,
							(SELECT vehicletype.name FROM vehicletype WHERE vehicletype.id=booking.vehicleType) AS carname,
								booking.vehiclenumber, (SELECT packages.name FROM packages WHERE packages.id=booking.package) AS Washpackage,
									booking.amount, booking.date FROM booking WHERE booking.worker=$id";
if(isset($_POST['book_today']))
{
	$sql="SELECT id, worker,status,payment, (SELECT customerLogin.firstname FROM customerLogin WHERE customerLogin.id=booking.cid) AS customer, 
						(SELECT login.workshop FROM login WHERE login.id=booking.did) AS dealer,
							(SELECT vehicletype.name FROM vehicletype WHERE vehicletype.id=booking.vehicleType) AS carname,
								booking.vehiclenumber, (SELECT packages.name FROM packages WHERE packages.id=booking.package) AS Washpackage,
									booking.amount, booking.date FROM booking WHERE booking.worker=$id AND DATE(date)=CURDATE()";
...
if(isset($_POST['book_done']))
{
	...
	$sql="SELECT id, worker,status,payment, (SELECT customerLogin.firstname FROM customerLogin WHERE customerLogin.id=booking.cid) AS customer, 
						(SELECT customerLogin.phone FROM customerLogin WHERE customerLogin.id=booking.cid) AS customer_phone,
							(SELECT vehicletype.name FROM vehicletype WHERE vehicletype.id=booking.vehicleType) AS carname,
								booking.vehiclenumber, (SELECT packages.name FROM packages WHERE packages.id=booking.package) AS Washpackage,
									booking.amount, booking.date FROM booking WHERE booking.worker=$id AND booking.status='Done'";
...
if(isset($_POST['payment']))
{
	...
	$sql="SELECT id, worker,status,payment, (SELECT customerLogin.firstname FROM customerLogin WHERE customerLogin.id=booking.cid) AS customer, 
						(SELECT customerLogin.phone FROM customerLogin WHERE customerLogin.id=booking.cid) AS customer_phone,
							(SELECT vehicletype.name FROM vehicletype WHERE vehicletype.id=booking.vehicleType) AS carname,
								booking.vehiclenumber, (SELECT packages.name FROM packages WHERE packages.id=booking.package) AS Washpackage,
									booking.amount, booking.date FROM booking WHERE booking.worker=$id AND booking.payment='$mode'";
